...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval (findM repeatSt xs) S.empty 
  where repeatSt a = do
          st <- get
          put (S.insert a st)
          return (S.member a st)
...
isHappy n =
  let
    square = join (*)
    work m = P.fromIntegral . sum $ map (square . digitToInt) (show' m)
    Full n' = firstRepeat . produce work $ n
  in n' == 1
