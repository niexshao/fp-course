...
findLeft f (ListZipper l a r) =
  case break f l of
    (_ , Nil) -> IsNotZ
    (bl, b :. br) -> IsZ (ListZipper br b (reverse bl ++ a :. r) )
...
findRight f (ListZipper l a r) =
  case break f r of
    (_, Nil) -> IsNotZ
    (bl, b :. br) -> IsZ (ListZipper (reverse bl ++ a :. l) b br)
...
moveLeftLoop (ListZipper Nil a r) = 
  let b :. l = reverse (a :. r)
  in ListZipper l b Nil
moveLeftLoop (ListZipper (b :. l) a r) = ListZipper l b (a :. r)
...
moveRightLoop (ListZipper l a Nil) =
  let b :. r = reverse (a :. l)
  in ListZipper Nil b r
moveRightLoop (ListZipper l a (b :. r)) = ListZipper (a :. l) b r
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper ls a (r :. rs)) = IsZ $ ListZipper ls r (a :. rs)
swapRight _ = IsNotZ
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n z | n > 0 = asMaybeZipper (moveLeftN (n-1)) (moveLeft z)
              | n < 0 = asMaybeZipper (moveLeftN (n+1)) (moveRight z)
              | otherwise = IsZ z
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n z | n > 0 = asMaybeZipper (moveRightN (n-1)) (moveRight z)
               | n < 0 = asMaybeZipper (moveRightN (n+1)) (moveLeft z)
               | otherwise = IsZ z
...
moveLeftN' n z = 
  case moveLeftN n z of
    IsNotZ -> Left (length $ if n > 0 then lefts z else rights z)
    IsZ z' -> Right z'
...
moveRightN' n z =
  case moveRightN n z of
    IsNotZ -> Left (length $ if n > 0 then rights z else lefts z)
    IsZ z' -> Right z'
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper ls a (r :. rs)) = end (ListZipper (a :. ls) r rs)
end z = z
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper (l :. ls) a rs) = start (ListZipper ls l (a :. rs))
start z = z
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper ls b rs) = ListZipper (b :. ls) a rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper ls b rs) = ListZipper ls a (b :. rs)
...
-- /Tip:/ Use `zipWith`
  ListZipper lf af rf <*> ListZipper l a r = ListZipper (zipWith ($) lf l) (af a) (zipWith ($) rf r)
...
instance Extend ListZipper where
  f <<= z =
    ListZipper (unfoldr (move moveLeft) z) (f z) (unfoldr (move moveRight) z)
    where
      move dir z' = (\a -> (f a, a)) <$> toOptional (dir z')
...
instance Traversable ListZipper where
  traverse f (ListZipper ls a rs) = ListZipper <$> traverse f ls <*> f a <*> traverse f rs
