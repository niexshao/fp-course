...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional =
  foldRight p (Full Nil)
  where p (Full a) (Full acc) = Full (a :. acc)
        p _        _          = Empty
...
-- find p =
--   foldLeft step Empty
--   where step Empty    x = if p x then Full x else Empty
--         step (Full x) _ = Full x
...
notReverse ::
  List a
  -> List a
notReverse = reverse -- error "not possible"
