...
data ParseError =
  UnexpectedEof
  | ExpectedEof Input
  | UnexpectedChar Char | Failed deriving Eq 
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = flbindParser character $ \c ->
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) $ \r ->
  case read r of 
    Full out -> valueParser out
    Empty -> failed
...
ageParser ::
  Parser Int
ageParser = flbindParser natural $ \r ->
  if r <= 0 then failed
            else valueParser r
...
surnameParser = do
  cap <- upper
  five <- thisMany 5 alpha
  rest <- list alpha
  return (cap :. five ++ rest)
